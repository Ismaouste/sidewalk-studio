...
- Moved the experience record from `/projects` to `/experience`, and pointed the crumb at the same word. The site had been calling one page two names: the menu said `Expériences` / `Experience`, while the URL and the breadcrumb said `Projets` / `Projects`, so a reader had to work out they were the same place. The page took the name it was already being called by. The two routes swapped rather than one being added — `/experience` had been a 301 into `/projects` and is now the page, `/projects` is now the 301 out — so every existing link keeps working and says where it went. Canonicals, the sitemap, the JSON-LD `BreadcrumbList`, the static export list, `/about`, and the unprefixed redirects all follow. One fault was found while swapping them and predates the change: the retired path resolved its target through `app()->getLocale()`, which is read from headers and cookies before the URL, so a French reader following `/fr/projects` was answered with `/en/experience` — losing the one thing that URL was certain about. The locale now comes from the route.
- Turned the case-studies CTA around. The primary action pointed back to the experience record, which is where most readers had just come from; a case study is the end of the reading funnel, not a step inside it, and someone who has just read one has the context to write. Contact leads, the experience link stays as the secondary.
