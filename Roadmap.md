...
### 017-editorial-back-office

- Keep the career chronology as rows in `experience_entries`, ordered by their own dates, with `date_label` as the display override for periods that are honestly imprecise.
- Keep both locales paired on every write. A position exists in two languages or in neither.
- Keep the questionnaire's questions declared in code and its prompts in the language files; an answer is a marginal note beside a spread, and unanswered is a resting state.
